...
/**
 * Quick Sort is a sorting algorithm that uses a divide and conquer strategy to sort the elements in an array.
 * It selects a pivot element from the array and partitions the array into two parts - one part with elements
 * smaller than the pivot and one part with elements larger than the pivot. After that, it sorts the two parts recursively.
 *
...
class QuickSort {
    ...
    /*
    *  Quick Sort has an average time complexity of O(n log n), which makes it very efficient for large sorting. However,
    *  keep in mind that an imprudent implementation may affect the stability of the sort or take up additional space in case of recursion.
    *
    */
}
